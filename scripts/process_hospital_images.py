...
def process_images(source_dir, dest_dir, prefix, watermark_path, count=6, max_dimension=1920, quality=80, opacity=0.44, size_percentage=20):
    ...
    # Filter out very small files (less than 100KB) just in case
    valid_files = []
    for f in files:
        f_path = os.path.join(source_dir, f)
        if os.path.getsize(f_path) > 100 * 1024:
    ...
    for idx, filename in enumerate(selected_files):
        ...
        output_filename = f"{prefix}_{idx + 1}_{os.path.splitext(filename)[0]}.webp"
...
if __name__ == '__main__':
    if len(sys.argv) < 4:
        print("Usage: python process_hospital_images.py <source_dir> <dest_dir> <prefix>")
        sys.exit(1)
    
    source_dir = sys.argv[1]
    dest_dir = sys.argv[2]
    prefix = sys.argv[3]
    watermark_path = "C:\\Users\\IT_COMMS\\GitHubProjects\\ACOB-Website\\public\\images\\acob-logo-dark.png"
    
    process_images(source_dir, dest_dir, prefix, watermark_path)
