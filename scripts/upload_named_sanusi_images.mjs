...
async function run() {
  ...
  const namedDir = "C:\\Users\\IT_COMMS\\Pictures\\New folder\\Sanusi_Project_Named";
  ...
  for (let i = 0; i < webpFiles.length; i++) {
    ...
    try {
      ...
      projectImages.push({
        ...
        alt: `Solar power and battery storage installations at Sir Mohammed Sanusi Hospital, Kano State (${filename.replace('.webp', '')})`,
        asset: {
          _type: 'reference',
          _ref: asset._id
        }
  ...
  // Find the asset that represents the main image (sir-mohammed-sanusi-hospital-1.webp)
  const mainImageAsset = projectImages.find(a => a.filename === "sir-mohammed-sanusi-hospital-1.webp");
  
  if (!mainImageAsset) {
    console.error("Error: Could not find main image sir-mohammed-sanusi-hospital-1.webp in uploaded assets.");
    process.exit(1);
  }

  const projectId = "healthcare-project-sir-mohammed-sanusi-hospital";
  ...
  await client
    ...
    .set({
      projectImage: {
        _type: 'image',
        alt: 'Sir Mohammed Sanusi Hospital 50 kWp Solar PV + Battery Storage Facility, Kano State, Nigeria',
