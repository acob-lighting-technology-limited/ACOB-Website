...
async function run() {
  ...
  console.log("Querying projects from Sanity with state 'Kano' or 'Kaduna'...");
  const projects = await client.fetch(`*[_type == "project" && (state match "Kano" || state match "Kaduna")]{
    _id,
    title,
    state,
    slug,
