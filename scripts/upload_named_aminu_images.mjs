...
async function run() {
  ...
  const namedDir = "C:\\Users\\IT_COMMS\\Pictures\\New folder\\Aminu_Project_Named";
  ...
  for (let i = 0; i < webpFiles.length; i++) {
    ...
    try {
      ...
      uploadedAssets.push({
        ...
        alt: `Solar power and battery storage installations at Aminu Kano Teaching Hospital, Kano State (${filename.replace('.webp', '')})`,
        asset: {
          _type: 'reference',
          _ref: asset._id
        }
  ...
  // Find the asset that represents the main image (aminu-kano-teaching-hospital-1.webp)
  const mainImageAsset = uploadedAssets.find(a => a.filename === "aminu-kano-teaching-hospital-1.webp");
  
  if (!mainImageAsset) {
    console.error("Error: Could not find main image aminu-kano-teaching-hospital-1.webp in uploaded assets.");
    process.exit(1);
  }

  const projectId = "healthcare-project-aminu-kano-teaching-hospital";
  ...
  await client
    ...
    .set({
      projectImage: {
        _type: 'image',
        alt: 'Aminu Kano Teaching Hospital 50 kWp Solar PV + Battery Storage Facility, Kano State, Nigeria',
  ...
  console.log(`Successfully updated ${projectId}! Images are renamed to match the project, cropped to landscape, and main image is set.`);
}
