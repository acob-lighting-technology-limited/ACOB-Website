...
async function run() {
  ...
  const namedDir = "C:\\Users\\IT_COMMS\\Pictures\\New folder\\Ikara_Project_Named";
  ...
  for (let i = 0; i < webpFiles.length; i++) {
    ...
    try {
      ...
      uploadedAssets.push({
        _type: 'image',
        filename: filename, // to trace which one is which
        alt: `Solar power and battery storage installations at Ikara Zonal Hospital, Kaduna State (${filename.replace('.webp', '')})`,
        asset: {
          _type: 'reference',
          _ref: asset._id
        }
  ...
  // Find the asset that represents the main image (ikara-general-hospital-6.webp)
  const mainImageAsset = uploadedAssets.find(a => a.filename === "ikara-general-hospital-6.webp");
  
  if (!mainImageAsset) {
    console.error("Error: Could not find main image ikara-general-hospital-6.webp in uploaded assets.");
    process.exit(1);
  }

  const projectId = "healthcare-project-ikara-general-hospital";
  console.log(`\nPatching Sanity document "${projectId}" with project-named images...`);
