...
async function run() {
  ...
  const namedDir = "C:\\Users\\IT_COMMS\\Pictures\\New folder\\Wase_Project_Named";
  ...
  for (let i = 0; i < webpFiles.length; i++) {
    ...
    try {
      ...
      projectImages.push({
        ...
        alt: `Solar power and battery storage installations at Abdullahi Wase Hospital, Kano State (${filename.replace('.webp', '')})`,
        asset: {
          _type: 'reference',
          _ref: asset._id
        }
  ...
  const videoSourcePath = "C:\\Users\\IT_COMMS\\OneDrive - ACOB Lighting Technology Limited\\ICT\\Projects\\Healthcare Sites\\Kano State\\⁠Abdulahi Wase Hospital\\20260605_083958000_iOS.MP4";
  ...
  if (fs.existsSync(videoSourcePath)) {
    ...
    try {
      ...
      videoAssetRef = {
        _type: 'video',
        title: 'Abdullahi Wase Hospital Solar System Walkthrough',
        alt: 'Video walkthrough of the completed solar installation at Abdullahi Wase Hospital, Kano.',
        asset: {
          _type: 'reference',
          _ref: asset._id
        }
  ...
  // Find the asset that represents the main image (abdullahi-wase-hospital-1.webp)
  const mainImageAsset = projectImages.find(a => a.filename === "abdullahi-wase-hospital-1.webp");
  
  if (!mainImageAsset) {
    console.error("Error: Could not find main image abdullahi-wase-hospital-1.webp in uploaded assets.");
    process.exit(1);
  }

  const projectId = "healthcare-project-abdullahi-wase-hospital";
  ...
  await client
    ...
    .set({
      projectImage: {
        _type: 'image',
        alt: 'Abdullahi Wase Hospital 50 kWp Solar PV + Battery Storage Facility, Kano State, Nigeria',
        asset: mainImageAsset.asset
      },
      projectContent: {
        description: 'healthcare1', // template 1
