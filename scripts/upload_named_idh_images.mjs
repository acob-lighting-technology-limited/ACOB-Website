...
async function run() {
  ...
  const namedDir = "C:\\Users\\IT_COMMS\\Pictures\\New folder\\IDH_Project_Named";
  ...
  for (let i = 0; i < webpFiles.length; i++) {
    ...
    try {
      ...
      projectImages.push({
        ...
        alt: `Solar power and battery storage installations at Infectious Disease Hospital Fagge, Kano State (${filename.replace('.webp', '')})`,
        asset: {
          _type: 'reference',
          _ref: asset._id
        }
  ...
  // Upload video walk-through
  const videoSourcePath = "C:\\Users\\IT_COMMS\\OneDrive - ACOB Lighting Technology Limited\\ICT\\Projects\\Healthcare Sites\\Kano State\\Infectious Disease Hospital, Fagge\\20260604_153209000_iOS.MOV";
  ...
  if (fs.existsSync(videoSourcePath)) {
    ...
    try {
      ...
      videoAssetRef = {
        _type: 'video',
        title: 'IDH Fagge Solar Power System Walkthrough',
        alt: 'Video walkthrough of the completed solar installation at Infectious Disease Hospital Fagge, Kano.',
        asset: {
          _type: 'reference',
          _ref: asset._id
        }
  ...
  // Find the asset that represents the main image (infectious-disease-hospital-fagge-1.webp)
  const mainImageAsset = projectImages.find(a => a.filename === "infectious-disease-hospital-fagge-1.webp");
  
  if (!mainImageAsset) {
    console.error("Error: Could not find main image infectious-disease-hospital-fagge-1.webp in uploaded assets.");
    process.exit(1);
  }

  const projectId = "healthcare-project-infectious-disease-hospital-fagge";
  ...
  // Clean helper key before writing
  const sanityImagesArray = allMedia.map(a => {
    if (a._type === 'video') return a;
    return {
      _type: 'image',
      alt: a.alt,
      asset: a.asset
  ...
  await client
    ...
    .set({
      projectImage: {
        _type: 'image',
        alt: 'Infectious Disease Hospital Fagge 50 kWp Solar PV + Battery Storage Facility, Kano State, Nigeria',
  ...
  console.log(`Successfully updated ${projectId}! Images are renamed to match the project, video walkthrough is registered, and main image is set.`);
}
